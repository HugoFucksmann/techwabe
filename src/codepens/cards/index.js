// pages of content
const pages = [
  ["Carrot", "Tomato", "Broccoli", "Cucumber", "Spinach"],
  ["Potato", "Pepper", "Eggplant", "Lettuce", "Onion"],
  ["Zucchini", "Cauliflower", "Kale", "Cabbage", "Mushroom"],
  ["Asparagus", "Artichoke", "Radish", "Beet", "Okra"],
  ["Brussels Sprouts", "Green Bean", "Butternut Squash", "Turnip", "Chard"],
];
// important elements
...
function addcontent() {
  for (let page = 0; page < pages.length; page++) {
    ...
    contentEl.innerHTML = pages[page]
      .map((item, index) => itemcard(item, index))
      .join("");
...
function itemcard(name, index) {
  return `
      <li style="transition: transform ${500 + index * 50}ms ease ${
    index * 10
  }ms">
        <div class="image"></div>
...
mainEl.addEventListener("scroll", (event) => {
  ...
  if (currentBox.top + 5 < mainBox.top && scrollamt > prevscroll) {
    ...
    currentpage = Math.min(
      currentpage + Math.ceil((scrollamt - prevscroll) / mainBox.height),
      pageEls.length - 1
    );
    updatedot();
    for (let page = 0; page < pageEls.length; page++) {
      pageEls[page].content.style.setProperty(
        "--translate",
        (page - currentpage) * 100 + "vh"
      );
    }
  }
  ...
  if (currentBox.top - 5 > mainBox.top && scrollamt < prevscroll) {
    ...
    currentpage = Math.max(
      currentpage - Math.ceil((-1 * (scrollamt - prevscroll)) / mainBox.height),
      0
    );
    updatedot();
    for (let page = 0; page < pageEls.length; page++) {
